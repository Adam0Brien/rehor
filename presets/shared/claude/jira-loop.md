...
Candidates in input prompt. Source + claim behavior: see workflow overlay.
...
1. **Claim**: `/claim-ticket <KEY>` — assign, In Progress, `task_add`. Sprint vs kanban: workflow overlay.

2. **Track**: After claim, `task_update` metadata if multi-repo:
   ```json
...
5. **Prepare repos**: `repo:` labels → match `project-repos.json`. Bare (`repo:insights-chrome`) or org-prefixed — resolved via upstream URLs. Fork workflow default: `url` = fork, `upstream` = original (PR target), `host` = "gitlab" if GL, `readonly` = read only.
   ...
   **Git identity**: Global config by `run.py`. Do NOT `git config --local` for identity/signing. Do NOT check `GPG_SIGNING_KEY` env. First commit fail on GPG/signing → immediately `git -c commit.gpgsign=false`. Do not probe `gpg`.
...
10. **Push + PR**: `/push-and-pr`. NEVER Bash `gh pr create` / `glab mr create`. If `/push-and-pr` fails, one `gh api` (GH) or `glab api` (GL) create — do not retry `gh pr create`. Push fail → `last_step = "push_failed"`, Jira comment, keep `in_progress`. Title ≤50 chars. Readonly → config changes in Jira comment.
...
12. **Post-PR**: `/post-pr <KEY>` — Jira → "Code Review", comment w/ PR links, Slack `pr_created`. NEVER `jira_transition_issue` yourself after opening a PR. `transition_id` is always a string. Also `/slack-notify` `needs_help` if blocked. Update linked w/ PR links.
